...
def units_obj():
    units_of_measurement = {'NPV': '(€)', 'Noise - Oss': '(dB)', 'Noise - Den Bosch': '(dB)',
                            'Bird Mortality': '(brids per year)',
                            'Particle Pollution': '(g per day)', 'Energy - Oss': '(GWh per year)',
                            'Energy - Den Bosch': '(GWh per year)', 'Project Time': '(years)'}
...
def bounds_cons():
    ...
    b4 = (1, 16)  # Number of turbines Den Bosch
    b5 = (50, 200)  # Turbine hub height Oss
    b6 = (50, 200)  # Turbine hub height Den Bosch
...
def get_SODO_stuff():
    ...
    result2 = minimize(obj_noise_disturbance_oss_alt, x0=np.array([3.5, 5, 1, 1, 50, 50]), bounds=bounds,
                       constraints=[], options={'maxiter': 1000})
    result3 = minimize(obj_noise_disturbance_bosch_alt, x0=np.array([3.5, 5, 1, 1, 50, 50]), bounds=bounds,
                       constraints=[], options={'maxiter': 1000})
    result4 = minimize(obj_new_bird_mort, x0=np.array([2, 2, 1, 1, 50, 50]), bounds=bounds,
                       constraints=[], options={'maxiter': 1000})
    ...
    result9 = minimize(obj_NPV_max, x0=np.array([3.5, 5, 1, 1, 50, 50]), bounds=bounds,
                       constraints=[], options={'maxiter': 1000})
    result10 = minimize(obj_noise_disturbance_oss_alt_max, x0=np.array([0.1, 0.1, 12, 20, 150, 150]), bounds=bounds,
                        constraints=[], options={'maxiter': 1000})
    result11 = minimize(obj_noise_disturbance_bosch_alt_max, x0=np.array([0.1, 0.1, 12, 20, 150, 150]), bounds=bounds,
                        constraints=[], options={'maxiter': 1000})
    result12 = minimize(obj_new_bird_mort_max, x0=np.array([2, 4, 12, 20, 150, 150]), bounds=bounds,
                        constraints=[], options={'maxiter': 1000})
    ...
    min_obj = [result9.fun, result2.fun, result3.fun, result4.fun, result5.fun, result14.fun, result15.fun, result8.fun]
    max_obj = [-result1.fun, -result10.fun, -result11.fun, -result12.fun, -result13.fun, -result6.fun, -result7.fun,
               -result16.fun]
